...
export const OBSIDIAN_TAG_KEYS = [
	OBSIDIAN_TAG_KEY_SINGULAR,
	OBSIDIAN_TAG_KEY_PLURAL,
];
export const OBSIDIAN_ALIAS_KEY_SINGULAR = "alias";
export const OBSIDIAN_ALIAS_KEY_PLURAL = "aliases";
export const OBSIDIAN_ALIASES_KEYS = [
	OBSIDIAN_ALIAS_KEY_SINGULAR,
...
export function formatYamlArrayValue(
	...
	const shouldRemoveEscapeCharactersIfPossible =
		...
		(format == NormalArrayFormats.MultiLine ||
			(format == SpecialArrayFormats.SingleStringToMultiLine &&
				value.length > 1));
	if (escapeNumericValues || shouldRemoveEscapeCharactersIfPossible) {
		for (let i = 0; i < value.length; i++) {
			...
			const shouldRequireEscapeOfCurrentValue =
				escapeNumericValues && isNumeric(currentValue);
			if (valueIsEscaped && shouldRequireEscapeOfCurrentValue) {
				...
				(valueIsEscaped && shouldRemoveEscapeCharactersIfPossible)
			) {
	...
	switch (format) {
		...
		case TagSpecificArrayFormats.SingleLineSpaceDelimited:
			...
			return (
				" " +
				convertStringArrayToSingleLineArray(value).replaceAll(", ", " ")
			);
	}
	/* eslint-enable no-fallthrough */
}

function getDefaultYAMLArrayValue(
	format: NormalArrayFormats | SpecialArrayFormats | TagSpecificArrayFormats
): string {
...
export function convertYAMLStringToArray(
	...
	if (delimiter.length > 1) {
		throw new Error(
			`The delimiter provided is not a single character: ${delimiter}`
		);
	}
...
export function escapeStringIfNecessaryAndPossible(
	...
	const basicEscape = basicEscapeString(
		value,
		defaultEscapeCharacter,
		forceEscape
	);
	...
	if (
		escapeWithOtherCharacter === value ||
		escapeWithOtherCharacter.length < escapeWithDefaultCharacter.length
	) {
...
function basicEscapeString(
	...
	if (
		...
		!substringHasDoubleQuote &&
		!substringHasColonWithSpaceAfterIt &&
		!forceEscape
	) {
		return value;
	}
